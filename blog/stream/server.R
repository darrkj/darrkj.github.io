...
create_data_file <- function(st, d1 = min(bike_stream$date), d2 = max(bike_stream$date)) {
  ...
  write.csv(dd, file = 'www/data2.csv', row.names = FALSE)
...
shinyServer(function(input, output) {
  ...
  output$networkPlot <- renderPrint({
    ...
    #z <- bike[bike$inn %in% input$m & bike$out %in% input$m, ]
    #val <- table(z$inn, z$out)
    #print(dput(input$daterange))
    if( length(input$m) > 1) {
      create_data_file(input$m, 
                       as.POSIXct(input$daterange[1]), 
                       as.POSIXct(input$daterange[2]))
      stream()
      #unlink('www/data2.csv')
    }
    #if( length(input$m) > 1) {
    #}
  })
