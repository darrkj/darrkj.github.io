...
```{r fig.width = 8, fig.height = 7, dev='jpeg'}
...
Time10a <- timer(roc1, 10, train$y, pred, gran = .1)
Time100a <- timer(roc1, 10, train$y, pred, gran = .01)
# I don't want to wait for this one to go through a loop.
Time1000a <- system.time(roc1(train$y, pred, gran = .001))[3]
```

Not a rigorous asymptotic analysis but we can see that there is almost a 10x jump from `r Time10a` to `r Time100a`. A 100x gives `r Time1000a`, which still seems to be linear still.
...
# actual was equal to the predicted in a vectorized manner. 
# We can take a similar action on each case since they are binary.
...
```{r Version2}
params2 <- function(act, pred, cutoff = .5) {
  pred <- ifelse(pred > cutoff, 1, 0)
  tp <- sum(act & pred)
  tn <- sum(!act & !pred)
  fp <- sum(!act & pred)
...
# How long does this take
system.time(roc2(test$y, pred, gran = .1))
system.time(roc2(test$y, pred, gran = .01))
# It already seems much faster
Time10b <- timer(roc2, 10, test$y, pred, gran = .1)
Time100b <- timer(roc2, 10, test$y, pred, gran = .01)

# It looks like we have a 10x gain in speed
Time1000b <- system.time(roc2(test$y, pred, gran = .001))[3]
```

This took `r Time10b`, `r Time100b` and `r Time1000b` for the ten, one hundred, and one thousand iteration version respectively.
...
# How long does this take
system.time(roc4(test$y, pred, gran = .1))
system.time(roc4(test$y, pred, gran = .01))

Time10d <- timer(roc4, 10, test$y, pred, gran = .1)
Time100d <- timer(roc4, 10, test$y, pred, gran = .01) 
Time1000d <- system.time(roc4(test$y, pred, gran = .001))[3]
```
...
lst <- c(Time10a, Time100a, Time1000a,
         ...
         Time10d, Time100d, Time1000d)

times <- matrix(lst, nrow = 3, ncol = 4, byrow = FALSE,
                dimnames = list(
                  c("10 Iters", "100 Iters", "1000 Iters"),
                  c("Baseline", "Vectorized", "Compiled", "Vect + Comp")))
times


benchmark(roc1(test$y, pred, gran = .1), 
          roc2(test$y, pred, gran = .1), 
          roc3(test$y, pred, gran = .1), 
          roc4(test$y, pred, gran = .1))
```