...
# This modification cut the load time in half, I would also bet it gets better 
# as there are more properties, it would make the inner loop longer.
add_nodes2 <- function(df, label, id, props = NULL, verbose = FALSE, na.act = F) {
  nodes <- list()
  # cannot have name of NA value
  miss <- !is.na(df[, id])
  uniq <- unique(df[miss, c(id, props), drop = FALSE])
